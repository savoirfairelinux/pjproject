...
static void apply_stream_socket_options(pj_sock_t sock, int type)
{
     ...
     * On Windows these need Windows 10 1703 for TCP_KEEPCNT and 1709 for the
     * other two, which is below the minimum this is built for.
     */
#ifdef TCP_KEEPIDLE
    set_tcp_opt(sock, pj_TCP_KEEPIDLE(), PJ_TCP_KEEPALIVE_IDLE);
#endif
#ifdef TCP_KEEPINTVL
    set_tcp_opt(sock, pj_TCP_KEEPINTVL(), PJ_TCP_KEEPALIVE_INTERVAL);
#endif
#ifdef TCP_KEEPCNT
    set_tcp_opt(sock, pj_TCP_KEEPCNT(), PJ_TCP_KEEPALIVE_COUNT);
#endif

    /*
     * Bound how long unacknowledged data may stay outstanding before the
     * connection is dropped. This is the timer that actually detects a peer
     * that stopped answering, since it fires whenever anything is in flight.
     *
     * Windows is left on its own default on purpose, even though it offers
     * an equivalent in TCP_MAXRT. Its default gives up sooner than the value
     * wanted here: measured on Windows 11, a segment that is never
     * acknowledged is retransmitted six times over 19.2s before the stack
     * aborts the connection, whereas the POSIX default is closer to fifteen
     * minutes. Setting TCP_MAXRT to PJ_TCP_RETRANSMIT_TIMEOUT would raise
     * 19.2s to 30s and so delay detection rather than speed it up.
     */
