 ...
 */
/*
 * Whether the per-socket keepalive options can be set with setsockopt().
 * Windows only grew them in Windows 10 1703 (TCP_KEEPCNT) and 1709
 * (TCP_KEEPIDLE, TCP_KEEPINTVL); older SDKs fall back to SIO_KEEPALIVE_VALS.
 */
#if defined(TCP_KEEPIDLE) && defined(TCP_KEEPINTVL) && defined(TCP_KEEPCNT)
#   define PJ_HAS_TCP_KEEPALIVE_OPT     1
#else
#   define PJ_HAS_TCP_KEEPALIVE_OPT     0
#endif
...
#if PJ_HAS_TCP_KEEPALIVE_OPT
    val = PJ_TCP_KEEPALIVE_IDLE;
    ka_status = pj_sock_setsockopt(sock, pj_SOL_TCP(), pj_TCP_KEEPIDLE(),
                                   &val, sizeof(val));
    if (ka_status == PJ_SUCCESS) {
        val = PJ_TCP_KEEPALIVE_INTERVAL;
        ka_status = pj_sock_setsockopt(sock, pj_SOL_TCP(), pj_TCP_KEEPINTVL(),
                                       &val, sizeof(val));
    }
    if (ka_status == PJ_SUCCESS) {
        val = PJ_TCP_KEEPALIVE_COUNT;
        ka_status = pj_sock_setsockopt(sock, pj_SOL_TCP(), pj_TCP_KEEPCNT(),
                                       &val, sizeof(val));
    }
#endif
...
#   define SIO_KEEPALIVE_VALS _WSAIOW(IOC_VENDOR, 4)
     ...
     */
#ifdef TCP_MAXRT
    val = PJ_TCP_RETRANSMIT_TIMEOUT;
    pj_sock_setsockopt(sock, pj_SOL_TCP(), TCP_MAXRT, &val, sizeof(val));
#endif
#else
    PJ_UNUSED_ARG(ka_status);
    val = PJ_TCP_RETRANSMIT_TIMEOUT * 1000;     /* milliseconds */
