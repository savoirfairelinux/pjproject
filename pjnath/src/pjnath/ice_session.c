...
static pj_bool_t check_ice_complete(pj_ice_sess *ice)
{
     ...
     * Skip components with no candidates (disabled m= sections), but require
     * at least one active component to prevent a false success when the
     * checklist is empty.
     */
    {
        ...
        if (active_cnt > 0 && i == ice->comp_cnt) {
