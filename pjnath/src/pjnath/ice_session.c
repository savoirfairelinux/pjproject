...
static pj_ice_sess_check* get_current_pending_check(pj_ice_sess *ice,
                                                     ...
                                                     int *current_check)
{
    ...
    for (int i = 0; i < ice->clist.count; ++i) {
        ...
        if (pj_sockaddr_cmp(remote_addr, &c->rcand->addr) == 0) {
            if (c->tdata == NULL ||
                (c->state < PJ_ICE_SESS_CHECK_STATE_SUCCEEDED && c->state >= PJ_ICE_SESS_CHECK_STATE_WAITING))
                continue;
            /* Match */
            check = c;
				 ...
				 pj_sockaddr_t* remote_addr)
{
    ...
    int current_check = -1;
    pj_ice_sess_check *check = get_current_pending_check(ice,remote_addr, &current_check);
	if (!check) {
		pj_grp_lock_release(ice->grp_lock);
		return;
	}
				       ...
				       pj_sockaddr_t* remote_addr)
{
    ...
    pj_grp_lock_acquire(ice->grp_lock);
    pj_ice_sess_check *check = get_current_pending_check(ice, remote_addr, NULL);
	if (!check) {
		pj_grp_lock_release(ice->grp_lock);
		return;
	}
